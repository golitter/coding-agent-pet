...
| 执行工具     | 奔跑 🏃     | 执行中...                                    |
| 任务完成     | 跳跃 🎉     | 搞定啦！✨                                   |
| 需要授权     | 等待 ⚠️     | 需要你的授权～（播放提示音；折叠时徽标变黄） |
| 出错         | 失败 💔     | 呜...出了点问题                              |
| 拖动宠物     | 方向奔跑 →← | —                                            |
| 会话结束     | 挥手 👋     | 下次见！♪                                    |
...
│   ├── bubble.js         #   对话气泡：显示/隐藏 + 折叠徽标 + 样式切换
│   ├── interaction-controller.js #   鼠标交互：悬停/拖动/三连击/穿透切换
│   ├── hit-test.js       #   精灵 alpha 与气泡/徽标命中检测
│   ├── permission-sound.js #   权限请求提示音与 WebAudio 兜底
│   └── style.css         #   样式（气泡/徽标/菜单/精灵）
├── src-tauri/            # 后端 (Rust)
│   ├── Cargo.toml        #   Rust 依赖
