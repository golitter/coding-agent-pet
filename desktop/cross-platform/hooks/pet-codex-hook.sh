...
# hooks/ → cross-platform/ → desktop/ → repo root
PLATFORM_DIR = Path('$SCRIPT_DIR').parent.resolve()
REPO_ROOT = PLATFORM_DIR.parent.parent  # desktop/cross-platform/hooks → repo root
...
sessions_dir = resolve(config.get('sessions_dir'), 'desktop', 'cross-platform', 'runtime', 'sessions')
...
# keeps Claude-style event names. Accept both shapes so either hook source works.
EVENT_ALIASES = {
...
try:
    with open('/tmp/kotori-pet-codex-hook.log', 'a') as log:
